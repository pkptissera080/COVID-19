    ...
    function initMap() {
        ...
        for (i = 0; i < locations.length; i++) {
            ...
            google.maps.event.addListener(marker, 'click', (function(marker, i) {
                return function() {
                    locations[i][6] === '1' ?
                        $("#mvactionbar").css('display', 'none') : $("#mvconfirm").css('display', '');
                    locations[i][6] === '0' ?
                        $("#mvactionbar").css('display', '') : $("#mvconfirm").css('display', 'none');
                    $("#imgdgmap").attr('src', '../res/pcrimg/' + locations[i][5]);
                    $("#Rejectid").val(locations[i][0]);
                    $("#Approveid").val(locations[i][0]);
                    ...
                    <tr>
                        <td>
                            <form action="" method="post">
                                <input type="text" style="display: none;" name="pcid" id="Rejectid">
                                <button type="submit" name="rejectReport" class="w3-button w3-red" style="border-radius: 5px;" id="mvreject" title="Reject">&#10008;</button>
                            </form>
                        </td>
                        <td>
                            <form action="" method="post">
                                <input type="text" style="display: none;" name="pcid" id="Approveid">
                                <button type="submit" name="approveReport" class="w3-button w3-green" style="border-radius: 5px;margin-right:5px;" id="mvapprove" title="Approve">&#10004;</button>
                            </form>
                        </td>
                    </tr>
                </table>
